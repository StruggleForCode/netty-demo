package com.leehao.nio.c4.no_block;
...
public class Server {
    public static void main(String[] args) throws IOException {
        ...
        scc.configureBlocking(false); // 非阻塞模式，影响的是accept方法
        ...
        while (true) {
            ...
            SocketChannel sc = scc.accept(); // 非阻塞方法，线程还会继续运行，如果没有连接建立，但是sc返回的是null
            if (sc != null) {
                // 每次建立的连接都放到这个集合里面，这样建立的连接都管理起来了
                log.debug("connected... {}", sc);
                // SocketChannel 也设置为非阻塞模式， 影响是下面的read方法
            ...
            for (SocketChannel channel : channels) {
                ...
                int read = channel.read(buffer);// 非阻塞，线程仍然继续运行，如果没有读到数据，read会返回0
                if (read > 0) {
                    // buffer切换到读模式
                    buffer.flip();
                    debugRead(buffer);
